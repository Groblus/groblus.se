...
Status, 2026-09-16: Groblus Spelhyllan is installed in Groblus Gamers with applications.commands only. The shared Worker and D1 have the real public verification key and guild ID. The exact interaction endpoint is exempted from browser login, Discord's signed PING was accepted, and an unsigned POST returned 401. Guild `/groblus` is registered with five subcommands; a real `/groblus profil` returned the expected private onboarding response. Linked-profile data-flow verification remains pending; see [acceptance](glantan-acceptance.md).
